...
int main() {
  ...
  auto sfb2x2 =
      make_tensor<SFBFrag2x2>(make_shape(make_shape(Int<128>{}, make_shape(Int<16>{}, Int<4>{})), Int<2>{}, Int<4>{}, Int<2>{}));
  ...
  std::printf("SFB(2x2) fragment encoded addresses (rank,u,seg)\n");
  for (int rank = 0; rank < 2; ++rank) {
    for (int u = 0; u < 2; ++u) {
      for (int seg = 0; seg < 4; ++seg) {
        uint32_t off =
