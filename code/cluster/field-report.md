# Cluster Perf Field Report (Published Canonical Package)

This is the published report package, not the operator entrypoint. For commands and artifact flow, start with `cluster/README.md`.
...
| Historical multi-node reference | `2026-02-10_full_suite_e2e_wire_qf_mon` is not materialized in this workspace; use `cluster/docs/advanced-runbook.md` for the current multi-node contract. |
...
| `04:39:17` | required artifact validation completed | error |
...
| Operator friction | full quick-friction battery executed | expected failures captured explicitly (`uv_torch_install`) | preserves operator visibility without false-red localhost status | [published/current/structured/2026-03-05_localhost_modern_profile_r24_full20b_localhost_quick_friction.json](published/current/structured/2026-03-05_localhost_modern_profile_r24_full20b_localhost_quick_friction.json) |
